...
\reportsubtitle{Proyecto de la asignatura Taller de Programación\\ Convocatoria ordinaria, curso 2025-2026} % Report subtitle, include new lines if needed
...
\reportdate{10 de febrero de 2026} % Report date, include new lines for additional information if needed
...
\begin{twothirdswidth} % Content in this environment to be at two-thirds of the whole page width
    ...
    \textcopyright~[2026] [Jorge Dueñas Lerín][Raúl Lara Cabrera] 
    ...
    \begin{tabular}{@{} L{0.05\linewidth} L{0.15\linewidth} L{0.6\linewidth} @{}} % Column widths specified here, change as needed for your content
        \toprule
        v1.1 & 10-02-2026 & Corregido el enunciado. Separado lógica de interfaz de usuario. \\
...
Para el desarrollo de este proyecto, podrás utilizar cualquier entorno de desarrollo integrado (IDE) que prefieras, aunque el soporte completo será ofrecido únicamente para \texttt{IntelliJ IDEA}, un potente IDE de \texttt{JetBrains} ampliamente utilizado en la industria y de código libre (licencia Apache 2.0) en su versión Community Edition, y para el cual, además, tenemos licencia educativa\sidecite{jetbrains-education} toda la comunidad universitaria.

El esqueleto del proyecto se encuentra disponible en un repositorio del \texttt{Gitlab} de la escuela. Todos los estudiantes tienen una cuenta creada en la plataforma y pueden acceder con su número de matrícula y su contraseña \url{https://gitlab.etsisi.upm.es/}.

El repositorio de la práctica pertenece a los profesores de la asignatura con lo que no podrás modificarlo. Este repositorio está disponible de forma pública y es software libre con lo que puedes crear tu propia copia y hacer modificaciones sobre ella. Esta acción acción recibe el nombre de \texttt{fork} (bifurcación). Para ello, sigue los siguientes pasos:

\begin{fullwidth}
1º Accede al repositorio del proyecto:

\href{https://gitlab.etsisi.upm.es/tallerdeprogramacion/2526/enunciado-extraordinaria}{https://gitlab.etsisi.upm.es/tallerdeprogramacion/2526/enunciado-extraordinaria}


2º Haz un fork del proyecto:

\includegraphics[width=\textwidth]{Images/tp-fork-ini-mark.png}

3º Configura la visibilidad y asócialo a tu usuario (Solo lo hará 1 persona del grupo)

\includegraphics[width=\textwidth]{Images/tp-fork-mark.png}
...
5º No te olvides de \textit{añadir al resto de integrantes como miembros} en tu repositorio (en \texttt{Manage - Members}).

\end{fullwidth}

Tras estas acciones podrás comenzar a utilizar el control de versiones git y sincronizar el trabajo sobre el código.

\begin{quote}
    \textbf{IMPORTANTE:} queda fuera del alcance de esta asignatura el uso de control de versiones. Existen prácticas opcionales en el \texttt{Moodle}. El uso de git es opcional aunque lo \textbf{recomendamos}.  
\end{quote}

Para finalizar esta sección, un último detalle. El proyecto se ha creado con \texttt{Maven}\sidecite{Redmond2008Jan}, un sistema de automatización de compilación y gestión de dependencias. Si utilizas \texttt{IntelliJ IDEA}, el IDE se encargará de importar el proyecto y configurar \texttt{Maven} automáticamente. Si trabajas desde la línea de comandos, puedes compilar el proyecto con el comando \texttt{mvn compile}. En el caso de que quieras gestionar las dependencias del proyecto manualmente, en la~\Cref{tab:dependencias-proyecto} se detallan las dependencias necesarias para el proyecto
\begin{margintable}
	\footnotesize
...
%TODO: TUI, primera vez que leo algo de esto. En Wikipedia se refiere a otro concepto.

\subsection{Descripción del proyecto}
La herramienta a desarrollar es lo que se conoce como una aplicación de línea de comandos (CLI, por sus siglas en inglés). La interfaz de usuario será textual, sin gráficos ni interacción con el ratón. El programa se ejecutará en la terminal de un sistema operativo, lo que se conoce como Text-based User Interface (TUI)\sidenote{Aunque pueda parecer algo del pasado, las TUI están a la orden del día. Echa un vistazo al siguiente \href{https://terminaltrove.com}{\color{blue}enlace}}. Para la realización del proyecto se proporciona un esquema de clases y métodos que los estudiantes deberán completar. Se usarán los conceptos de programación ya trabajados en la asignatura de Fundamentos de Programación. Para la realización del proyecto se proporciona un esquema de clases y métodos que los estudiantes deberán completar. Se usarán los conceptos de programación ya trabajados en la asignatura de Fundamentos de Programación.
...
\begin{table*} % Use the table* environment for full width tables
    ...
    \begin{tabular}{C{0.03\linewidth} L{0.3\linewidth} L{0.55\linewidth}}
        ...
        \midrule
        \textit{1} & \texttt{Receta} & Representa una receta con un nombre, ingredientes e instrucciones. Permite agregar ingredientes e instrucciones. Proporciona métodos para obtener información sobre la receta.\\
        ...
        \textit{3} & \texttt{PlanificadorSemanal} & Permite planificar recetas para cada día de la semana así como guardar el plan semanal en un archivo.\\
        \textit{4} & \texttt{InterfazUsuario} & Gestiona la interacción con el usuario a través de un menú en consola.\\
...
El número máximo de ingredientes e instrucciones que puede tener una receta se especifican como párametros de ejecución. La clase \texttt{Receta} define constantes para representar los posibles resultados de las operaciones de agregado\sidenote{Este enfoque permite separar la lógica de negocio (en \texttt{Receta}) de la presentación (en \texttt{InterfazUsuario}), facilitando el mantenimiento y las pruebas del código.}:

\begin{code}
\caption{Constantes de códigos de error en la clase Receta.}\label{lst:codigos-error-receta}
\begin{minted}
[
frame=lines,
framesep=1mm,
...
\mintinline[fontsize=\footnotesize]{java}|public int agregarIngrediente(String ingrediente)|

que recibe como parámetros el nombre del ingrediente. La funcionalidad de agregar instrucción se encapsula en el método

\mintinline[fontsize=\footnotesize]{java}|public int agregarInstruccion(String instruccion)|

que recibe como parámetro la instrucción.

Ambos métodos devuelven un código entero que indica el resultado de la operación\sidenote{\texttt{EXITO} si se ha realizado la operación correctamente, \texttt{ERROR\_VALOR\_INVALIDO} si el parámetro es null o vacío, \texttt{ERROR\_INGREDIENTES\_COMPLETOS} o \texttt{ERROR\_INSTRUCCIONES\_COMPLETAS} si se ha superado el máximo permitido}. Es responsabilidad de la clase \texttt{InterfazUsuario} interpretar estos códigos y mostrar los mensajes apropiados al usuario. Por ejemplo, cuando se intenta agregar más ingredientes o instrucciones de los permitidos, \texttt{InterfazUsuario} mostrará:
...
Para cargar desde un fichero y generar una receta tendremos un método estático que espera recibir un \texttt{BufferedReader} ya abierto desde el que se están cargando recetas (ver \texttt{LibroRecetas}).

\mint[fontsize=\footnotesize]{java}|public static Receta fromBufferedReader(BufferedReader reader, int maxIngredientes, int maxInstrucciones)|

Este método lee la información de una receta desde el \texttt{BufferedReader} proporcionado y devuelve un objeto \texttt{Receta} con la información leída. Los parámetros \texttt{maxIngredientes} y \texttt{maxInstrucciones} indican el número máximo de ingredientes e instrucciones que puede contener la receta. Si el archivo contiene más ingredientes o instrucciones de los especificados, la información se truncará.
...
\mintinline[fontsize=\footnotesize]{java}|public int agregarReceta(Receta receta)|

que recibe como parámetro\sidenote{Será tu responsabilidad comprobar si la receta es un valor nulo.} la receta a añadir. El método devuelve un valor entero que indica si la operación se ha realizado correctamente. Los posibles códigos son:

\begin{code}
\caption{Constantes de códigos de error en la clase LibroDeRecetas.}\label{lst:codigos-error-catalogo}
\begin{minted}
[
frame=lines,
framesep=1mm,
...
Si se intenta añadir una receta a un libro que ya está completo, el método devuelve \texttt{ERROR\_DEMASIADAS}. Es responsabilidad de la clase \texttt{InterfazUsuario} interpretar este código y mostrar el mensaje apropiado al usuario:
...
El mecanismo de inserción de recetas en el libro no está especificado, por lo que puedes elegir la estructura de datos que consideres más adecuada para almacenar las recetas. Ten en cuenta que el libro de recetas tiene un tamaño fijo, que se especifica como parámetro de ejecución. Además, ten en cuenta que en el catálogo se permite borrar actividades.

\paragraph{Eliminar receta}

La funcionalidad que nos permite eliminar una receta se encapsula en el método

\mintinline[fontsize=\footnotesize]{java}|public boolean eliminarReceta(Receta seleccionada)|

que recibe como parámetro la receta a eliminar. Si la receta no se encuentra\sidenote{Encontrar una receta significa que son \textbf{el mismo} objeto.} en el libro, el método devuelve \texttt{false}. En caso contrario, devuelve \texttt{true}.
...
El componente \texttt{PlanificadorSemanal} es el encargado de planificar las comidas para cada día de la semana y de guardar el plan semanal en un archivo. Para simplificar este módulo solo se permitirá planificar una receta por día de la semana.

A continuación, nos centramos en cada una de las funcionalidades que debe implementar este componente.
...
Para no tener los números del 0 al 6 repartidos por el código, es recomendable definir constantes para representar cada día de la semana. Además se crearé un array de cadenas de texto con el nombre de los días de la semana para mostrar por pantalla.
...
Como la longitud del nombre de cada receta puede variar se va a especificar una longitud fija de 12 para cada columna. Si el nombre de la receta es menor que 12, se rellenará con espacios a la derecha. Si el nombre de la receta es mayor que 11, se truncará a 10 caracteres y se añadirá un punto al final para indicar que se ha truncado.

\begin{code}
\caption{Constantes de ancho columna en la clase PlanSemanal.}\label{lst:ancho-columna}
\begin{minted}
[
frame=lines,
framesep=1mm,
...
public static final int MAX_RECETA_NOMBRE = 10;
\end{minted}
\end{code}
...
Método que muestra el mensaje \texttt{s} por pantalla y lee una cadena de texto introducida por el usuario. El método no permite al usuario leer cadenas vacías. El método devuelve la cadena de texto leída.
...
Lo métodos de la clase utilidades deben usar las constantes de la clase \texttt{PlanificadorSemanal} para comprobar los días de la semana.
...
Ten en cuenta que debemos tener en cuenta los posibles códigos de error de la clase \texttt{Receta} al agregar ingredientes e instrucciones, y el código de error de la clase \texttt{LibroDeRecetas} al agregar la receta al libro, para mostrar los mensajes de error correspondientes al usuario (ver~\Cref{lst:error-maximo-receta} y~\Cref{lst:error-maximo-libro}).
...
% Los dos siguientes son en los que hay diferencias de opinión (y yo opino que están mal :) )
...
\begin{itemize}
	...
	\item Plagio: tu implementación contiene código copiado de otras fuentes.
